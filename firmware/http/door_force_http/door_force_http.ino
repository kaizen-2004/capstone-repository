...
static const char* FIRMWARE_VERSION = "door_force_http_v2_provision";
...
static const uint32_t SENSOR_REPORT_INTERVAL_MS = 3000;
...
void setupProvisionServerRoutes() {
  ...
  provisionServer.on("/", HTTP_GET, []() {
    ...
    provisionServer.send(
        200, "text/html",
        "<html><body><h3>door_force setup node ready</h3><p>Use "
...
void loop() {
  ...
  if ((now - lastReadingMs) >= SENSOR_REPORT_INTERVAL_MS) {
    ...
    const char* eventCode = simulated > 0.80f ? "DOOR_FORCE" : "ENTRY_MOTION";
