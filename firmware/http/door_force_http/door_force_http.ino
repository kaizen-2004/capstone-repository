...
static const char* WIFI_HOSTNAME = "door-force";
...
bool initImuIfNeeded() {
  ...
  uint32_t now = millis();
  if ((uint32_t)(now - lastImuInitAttemptMs) < IMU_REINIT_INTERVAL_MS) {
    return false;
  }
  ...
  Serial.println("[IMU] LSM6DS3 not detected; retrying...");
  return false;
}
...
float accelMagnitudeG(const ImuSample& sample) {
  return sqrtf((sample.ax * sample.ax) + (sample.ay * sample.ay) + (sample.az * sample.az));
}

float gyroMaxDps(const ImuSample& sample) {
  float ax = fabsf(sample.gx);
...
void loop() {
  ...
  if (imuReady && (uint32_t)(now - lastReadingMs) >= SENSOR_REPORT_INTERVAL_MS) {
    lastReadingMs = now;
    ...
    if (readImuSample(sample)) {
      ...
      if (!calibrated) {
        ...
        if (calibrationCount >= CALIBRATION_SAMPLES) {
          baselineMagG = calibrationSumMagG / static_cast<float>(calibrationCount);
          calibrated = true;
